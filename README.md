# FocusHive
"Find the best place to focus right now."

FocusHive is a mobile application developed as a semester project. It goes beyond standard campus maps by allowing students to find, rate, and share real-time study environments based on crucial focus factors.
...
## Architecture
FocusHive utilizes a modern, decoupled mobile architecture:
...
* **Backend as a Service (BaaS):** Firebase.
    ...
    * **Database:** Cloud Firestore (NoSQL) structured into `Users`, `Locations`, and `Ratings` collections for real-time data syncing.
...
You must configure the Firebase and API keys. Create a `.env` file in the root directory (do not commit this file to version control):
```text
...
Link the native Apple dependencies using CocoaPods:
```bash
cd ios
pod install
cd ..
...
Start the Metro Bundler and launch the application on an emulator or physical device:
